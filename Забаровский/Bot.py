...
class Bot(object):
    ...
    def answer_message(self, message):
        ...
        self.write_to_file(message, answer)
        return answer

    def create_answers(self, mess_type):
        greetlist = ['Привет', 'Здравствуй', 'Доброго времени суток']
        byelist = ['Пока', 'До свидания', 'До встречи','Ещё увидимся']
        answers_type = {'greeting': [greet + ', ' + self.user_name + " !" for greet in greetlist],
                        'bye': [bye + ', ' + self.user_name + " !" for bye in byelist],
                        'howareyou': ['Хорошо', 'Нормально', 'Замечательно', 'Прекрасно','Работаю'],
                        'commands': [
                            'Я умею делать арифметические операции: например "Посчитай 2+2", "Сложи 4 и 7","Сколько будет 9.536 поделить на -3". Могу вывести дату и время, побеседовать и рассказать анекдот'],
    ...
    def check_message_type(self, message):
        phrases_type = {'greeting': ['Привет', 'Здравствуй', 'Добрый день', 'Добрый вечер', 'Доброе утро','привет','здравствуй'],
                        'bye': ['Пока', 'До свидания', 'До встречи'],
                        'howareyou': ['Как ты', 'Как дела', 'Как поживаешь'],
                        'commands': ['Что ты умеешь', 'Что ты можешь', 'Расскажи о себе', 'Помощь','помощь'],
                        'date': ['Дата','дата', 'число'],
                        'time': ['Время','время', 'времени'], 'anekdot': ["анекдот","Анекдот"], 'thankyou': ['Спасибо','благодарю','спасибо','Благодарю']}
    ...
    def write_to_file(self, input, output):
        with open("history.txt", "a") as fileObj:
            fileObj.write(self.user_name + ': ' + input + '           ' + strftime("%Y-%m-%d %H:%M:%S", localtime()) + '\n')
            fileObj.write('Бот: ' + output + '           ' + strftime("%Y-%m-%d %H:%M:%S", localtime()) + '\n')
